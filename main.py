...
# Function to handle the /Manually command
async def manually_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("You are using the bot manually. Perform your tasks here.")
    # Add your manual functionality code here
...
# Function to handle the /Auto command
async def auto_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("Automated mode activated. The bot will now perform tasks automatically.")
    # Add your automated functionality code here
    # Upload command handler
    @bot.on_message(filters.command(["upload"]))
    async def upload(bot: Client, m: Message):
        editable = await m.reply_text('𝐓𝐨 𝐃𝐨𝐰𝐧𝐥𝐨𝐚𝐝 𝐀 𝐓𝐱𝐭 𝐅𝐢𝐥𝐞 𝐒𝐞𝐧𝐝 𝐇𝐞𝐫𝐞 📄')
